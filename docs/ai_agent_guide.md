...
- **活用 Meta Data**：若遇到 Schema 未定義的細節（如：特定工藝的材料配方），應將其格式化為 JSON 存入 `meta_data`。必須包含 `semantic_summary` (純文字摘要) 以及規格書中定義的標準鍵值 (如 `media_info`, `tech_specs`)。
- **衝突處理規範**：若發現資料來源間有衝突，禁止 AI 自行決定真相。應分開錄入多筆資料，並在 `meta_data` 標記衝突關係。
- **支系管理**：錄入部落前，應檢查其所屬的子族群（支系）是否存在於 `ethnic_groups`，確保 `parent_id` 正確。

---

## 9. AI-Native 維護與狀態傳承規範 (AI-Native Maintenance)

為了確保專案在多個 AI 代理程式間無縫銜接，必須遵循以下「AI 本位」準則：

### 9.1 狀態傳承 (Context Handoff)
- **現狀摘要**：每一階段任務結束前，AI 應更新 `build_log.md` 中的「當前狀態快照」，包含：已建模族群、數據完整度、當前待辦與已辨識的風險。
- **快速錨點**：AI 應隨時檢視 `VERSION` 與 `TOC.md` 以校準其產出的架構層次。

### 9.2 自動化約束 (Automated Guardrails)
- **腳本驗證**：在執行任何 `INSERT/UPDATE` 後，應執行 `scripts/validation/` 下的檢查工具，驗證 JSON 格式與關聯完整性。
- **禁止漂移**：若 AI 對規格有不同解讀，應先更新 `db_design_spec.md` 並取得確認，禁止直接在 DB 中實作非標格式。

### 9.3 決策紀錄 (Reasoning Log)
- **錄入原因**：對於複雜的文化連結 (`cultural_links`) 或平行敘事處理，AI 應在 `meta_data` 中加入 `reasoning` 欄位，記錄其選擇特定來源或邏輯的原因。
